...
command = f"sudo -E nice -n -20 taskset -c 0-$(nproc --ignore=0) ~/Thesis_work/code/main_dubin_check.exe"
# command = f"~/Thesis_work/code/main_dubin_check.exe"
...
activate = simulation_params['activate_simulation']
analyze = params['SetPointController']['analyze']

if activate == 1:
    # Run the python file animate_all_data.py
    subprocess.run(["python3", "animate_all_data.py"], check=True)

    animate_process = subprocess.run(
            ["python3", "animate_all_data.py"],
            check=True,
            text=True,
            capture_output=True
        )
    print(animate_process.stdout)

if analyze == 1:
    # Run the python file analyze.py
    subprocess.run(["python3", "ControlData/analyze.py"], check=True)

    analyze_process = subprocess.run(
            ["python3", "ControlData/analyze.py"],
            check=True,
            text=True,
            capture_output=True
        )
